...
install_metallb()
{
	...
	kubectl create secret generic -n metallb-system memberlist --from-literal=secretkey="$(openssl rand -base64 128)" ./srcs/log/logs 2>&1
}
...
echo "${ORANGE}Please give the permission to work (user42): ${NC}"
sudo ls / > /dev/null
